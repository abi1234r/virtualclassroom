...
function startRecording() {
    if (!localStream) {
        showToast("Cannot start recording without media access.");
        return;
    }

    const audioTrack = localStream.getAudioTracks()[0];
    const videoTrack = (screenStream && screenStream.getVideoTracks()[0]) || localStream.getVideoTracks()[0];

    if (!audioTrack && !videoTrack) {
        showToast("Nothing available to record.");
        return;
    }

    const recordingStream = new MediaStream();
    if (audioTrack) {
        recordingStream.addTrack(audioTrack.clone ? audioTrack.clone() : audioTrack);
    }
    if (videoTrack) {
        recordingStream.addTrack(videoTrack.clone ? videoTrack.clone() : videoTrack);
    }
    ...
    if (recordBtn) {
        ...
        recordBtn.innerHTML = '<span class="material-symbols-rounded">stop_circle</span>';
    }
...
function stopRecording() {
    ...
    if (recordBtn) {
        ...
        recordBtn.innerHTML = '<span class="material-symbols-rounded">fiber_manual_record</span>';
    }
    showToast('Recording stopped. Preparing file...');
}
...
function downloadRecording() {
    ...
    link.href = url;
    link.download = `meeting-${ROOM}-${Date.now()}.webm`;
    document.body.appendChild(link);
    link.click();
    link.remove();

    setTimeout(() => URL.revokeObjectURL(url), 1000);
    showToast('Meeting recording saved to your device.');
}
